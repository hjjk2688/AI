...
---
#### CNN Bais(편향)
CNN의 Conv2D 레이어에서, 편향(Bias)은 출력 피처 맵(Feature Map) 당 하나씩 존재합니다.

그리고 출력 피처 맵의 개수는 사용된 필터(Filter)의 개수와 같고, 이것이 바로 출력의 깊이(Depth)가 됩니다.

따라서 "출력 깊이마다 하나씩 있다" 라고 이해하면 된다.

- 부연 설명
    1. 하나의 필터가 입력 데이터 전체를 훑으며 합성곱 연산을 수행하여, 하나의 2차원 피처 맵을 만듭니다.
    2. 이 피처 맵의 모든 원소에 동일한 하나의 편향(Bias) 값이 더해집니다.
    3. 32개의 필터를 사용했다면, 이 과정이 32번 반복되어 각각 다른 32개의 피처 맵과, 그에 해당하는 32개의 편향
      값이 사용됩니다.

>결과적으로, 출력의 깊이가 32라면, 편향도 32개가 필요합니다.
